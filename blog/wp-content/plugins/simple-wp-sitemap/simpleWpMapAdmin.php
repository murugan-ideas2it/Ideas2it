...
<div class="wrap">
    ...
    <ul>
        <li><?php printf('%1$s <a href="%2$s">%2$s</a>', __('Xml sitemap:', 'simple-wp-sitemap'), esc_url($ops->getSitemapUrl('xml'))); ?></li>
        <li><?php _e('Html sitemap:', 'simple-wp-sitemap'); ?>
            <?php if (get_option('simple_wp_block_html')) {
                _e('(disabled)', 'simple-wp-sitemap');
            } else {
                printf('<a href="%1$s">%1$s</a>', esc_url($ops->getSitemapUrl('html')));
            } ?>
        </li>
    </ul>
    ...
    <form method="post" action="<?php $ops->printSubmitUrl(); ?>" id="simple-wp-sitemap-form">
        ...
        <table id="sitemap-normal" class="widefat form-table">
            ...
            <tr>
                <td>
                    <textarea rows="7" name="simple_wp_other_urls" placeholder="<?php esc_attr_e('http://www.example.com/a-page/', 'simple-wp-sitemap'); ?>" class="large-text code" id="swsp-add-pages-textarea"><?php $ops->printUrls('simple_wp_other_urls'); ?></textarea>
                </td>
            </tr>
            ...
            <tr>
                <td>
                    <?php _e('Add pages you want to block from showing up in the sitemaps. Same as above, just paste every link on a new row. (Hint: copy paste links from one of the sitemaps to get correct urls).', 'simple-wp-sitemap'); ?>
                </td>
            </tr>
            <tr>
                <td>
                    <textarea rows="7" name="simple_wp_block_urls" placeholder="<?php esc_attr_e('http://www.example.com/block-this-page/', 'simple-wp-sitemap'); ?>" class="large-text code"><?php $ops->printUrls('simple_wp_block_urls'); ?></textarea>
                </td>
            </tr>
            ...
            <tr>
                <td>
                    <p><input type="checkbox" name="simple_wp_disp_categories" id="simple_wp_cat" <?php $ops->printChecked('simple_wp_disp_categories'); ?>><label for="simple_wp_cat"> <?php _e('Include categories', 'simple-wp-sitemap'); ?></label></p>
                    <p><input type="checkbox" name="simple_wp_disp_tags" id="simple_wp_tags" <?php $ops->printChecked('simple_wp_disp_tags'); ?>><label for="simple_wp_tags"> <?php _e('Include tags', 'simple-wp-sitemap'); ?></label></p>
                    <p><input type="checkbox" name="simple_wp_disp_authors" id="simple_wp_authors" <?php $ops->printChecked('simple_wp_disp_authors'); ?>><label for="simple_wp_authors"> <?php _e('Include authors', 'simple-wp-sitemap'); ?></label></p>
                </td>
            </tr>
                ...
                <td>
                    <select name="simple_wp_block_html" id="simple_wp_block_html">
                        <?php foreach (array('' => __('Enable', 'simple-wp-sitemap'), '1' => __('Disable', 'simple-wp-sitemap'), '404' => __('Disable and set to 404', 'simple-wp-sitemap')) as $key => $val) { ?>
                            <option value="<?php echo $key; ?>" <?php $ops->printSelected('simple_wp_block_html', $key); ?>><?php echo $val; ?></option>
                        <?php } ?>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                <td>
                    <input type="checkbox" name="simple_wp_attr_link" id="simple_wp_check" <?php $ops->printChecked('simple_wp_attr_link'); ?>><label for="simple_wp_check"> <?php _e('Add "Generated by Simple Wp Sitemap" link at bottom of sitemap.html', 'simple-wp-sitemap'); ?></label>
                </td>
            </tr>
            <tr>
                <td>
        ...
        <table id="sitemap-advanced" class="widefat form-table">
            ...
            <tr>
                <td>
                    <ul id="sitemap-display-order">
                        <?php $orderArray = $ops->checkOrder(get_option('simple_wp_disp_sitemap_order'));
                        if (!$orderArray) {
            ...
            <tr>
                <td>
                    <b><?php _e('Order posts and pages after', 'simple-wp-sitemap'); ?></b>
                    <select id="simple_wp_order_by" name="simple_wp_order_by">
                        <option value=""><?php _e('Posted date', 'simple-wp-sitemap'); ?></option>
                        <option value="modified" <?php $ops->printSelected('simple_wp_order_by', 'modified'); ?>><?php _e('Last updated date', 'simple-wp-sitemap'); ?></option>
                    </select>
                </td>
            </tr>
            <tr>
                <td>
                    <input type="button" id="sitemap-defaults" class="button-secondary" value="<?php esc_attr_e('Restore order defaults', 'simple-wp-sitemap'); ?>">
                </td>
            </tr>
        ...
        <table id="sitemap-premium" class="widefat form-table">
            ...
            <tr>
                <td>
                    <ul class="simple-wp-sitemap-includes">
                        ...
                        <li><?php _e('Split sitemaps into multiple pages', 'simple-wp-sitemap'); ?></li>
            ...
            <tr>
                <td>
                    <b><?php _e('If you have a premium code, enter it here to upgrade', 'simple-wp-sitemap'); ?></b><br><br>
                    <input type="text" id="upgradeField"> <span class="button-secondary" id="upgradeToPremium"><?php _e('Upgrade', 'simple-wp-sitemap'); ?></span>
                    <span id="swpErrorText"><?php $ops->printError(); ?></span>
                </td>
            </tr>
            <tr>
                <td>
                    <hr><br>
                    <b><?php _e('Available at', 'simple-wp-sitemap'); ?></b>: <a target="_blank" href="<?php echo esc_url('https://www.webbjocke.com/downloads/simple-wp-sitemap-premium/'); ?>"><?php _e('webbjocke.com/downloads/simple-wp-sitemap-premium', 'simple-wp-sitemap'); ?></a>
                </td>
            </tr>
    ...
    <form method="post" action="<?php $ops->printSubmitUrl(); ?>&amp;tab=3" class="table-hidden" id="simpleWpHiddenForm"></form>
